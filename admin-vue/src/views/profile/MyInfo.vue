    ...
    <el-card shadow="never" v-loading="loading">
      <template v-if="info">
        <el-descriptions :column="2" border size="default" class="info-descriptions">
          ...
          <el-descriptions-item label="工号/学号" :span="1">{{ info.userNo }}</el-descriptions-item>
          ...
          <el-descriptions-item label="邮箱" :span="1">{{ info.email || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="状态" :span="1">
            <el-tag :type="info.status === '正常' || info.status === '在职' || info.status === '在读' ? 'success' : 'danger'" size="small">{{ info.status }}</el-tag>
          </el-descriptions-item>
