...
const props = defineProps({
  title: { type: String, default: '智慧助手' },
  subtitle: { type: String, default: '' },
  welcomeText: { type: String, default: '我可以帮你查询信息、分析数据、管理事务' },
  quickQuestions: { type: Array, default: () => [] },
  sessionPrefix: { type: String, default: 'chat' },
  chatStream: { type: Function, required: true },
  getSessions: { type: Function, required: true },
  getChatHistory: { type: Function, required: true },
...
function generateSessionId() {
  return props.sessionPrefix + '_' + Date.now() + '_' + Math.random().toString(36).substring(2, 9)
}
...
async function loadSessions() {
  ...
  try {
    const res = await props.getSessions()
...
async function selectSession(session) {
  ...
  loading.value = true
  try {
    const res = await props.getChatHistory(session.sessionId, 200)
...
function handleSessionDelete(sessionIdToDelete) {
  pendingDeleteSessionId = sessionIdToDelete
...
async function confirmDelete() {
  ...
  try {
    await props.deleteSession(pendingDeleteSessionId)
